...
class TestConnectHugeMuf(ServerTestBase):
    ...
    extra_params = {'command_burst_size': '1000000', 'commands_per_time': 10000,
                    'max_output': 10005 * 50 * 6}
    def test(self):
        muf_program = b": main \n" + b"  1\n" * self.line_count + b";\n"
        string = (CONNECT + 
b"""@program test.muf
...
""")
        ...
        logging.error('OutputFlushed count %s', result.count(b"<Output Flushed>"))
        if result.count(b"<Output Flushed>") == 0:
            self.assertEqual(
            ...
            self.assertFalse(self.expect_no_truncate)
        self.assertTrue(
...
class TestConnectHugeMufTruncateNoSSL(TestConnectHugeMufTruncate):
    use_ssl = False
